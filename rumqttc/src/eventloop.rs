...
use crate::{
    ...
    AsyncReadWrite, ClientState, Connect, ConnectReturnCode, Control, Disconnect, IOEvent, Log,
    Message, MqttOptions, Network, NetworkOptions, Packet, PubAck, Publish, SubAck,
    SubscribeReasonCode, Transport, UnsubAck, UnsubAckReason,
};
...
async fn network_connect(
    ...
    if matches!(options.transport(), Transport::Unix) {
        ...
        let network = Network::new(Box::new(socket));
        return Ok(network);
    }
    ...
    let network = match options.transport() {
        Transport::Tcp => Network::new(tcp_stream),
        #[cfg(any(feature = "use-rustls-no-provider", feature = "use-native-tls"))]
        ...
        Transport::Ws => {
            ...
            Network::new(Box::new(WsStream::new(socket)))
        }
        #[cfg(all(feature = "use-rustls-no-provider", feature = "websocket"))]
